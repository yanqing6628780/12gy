                    ...
                    <div class="col-md-12">                    
                        <form id="search" class="form-inline">
                            <div class="form-group">                                
                                <input name="q" type="text" value="<?=$q?>" class="form-control" placeholder="内容搜索">
                            </div>
                                <input name="rel_id" type="hidden" value="<?=$rel_id?>" >
                    ...
                    <table class='table table-striped table-bordered table-hover'>
                        ...
                        <?php foreach($result as $key => $row):?>
                            <tr>
                                ...
                                <td>
                                    <?php if( chk_perm_to_bool('v_campaign_admin') || chk_perm_to_bool('c_campaign_admin') || chk_perm_to_bool('b_campaign_admin') || chk_perm_to_bool('d_campaign_admin') ):?>
...
function del(id){
    $.ajax({
        url: '<?=site_url($controller_url."comment_del")?>',
        type: 'POST',
        dataType: 'json',
        data: {id: id},
        success: function(data, textStatus, xhr) {
            alert('删除成功');
            LoadPageContentBody('<?=site_url($controller_url2)?>',{rel_id: <?=$rel_id?>, campaign_name: "<?=$campaign_name?>", type: <?=$type?>});
        }
    });   
}
function infoQuery() {
