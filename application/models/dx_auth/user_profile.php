...
class User_Profile extends CI_Model 
{
	...
	function set_profile($user_id, $data)
	{
		$this->db->where('user_id', $user_id);
		//将数组转换为用逗号链接的字符串
		$data['service_items'] = isset($data['service_items']) ? implode(',', $data['service_items']) : '';
		$data['star_service_items'] = isset($data['star_service_items']) ? implode(',', $data['star_service_items']) : '';
